...
RT_PROGRAM void pinhole_camera()
{
  ...
  output_buffer[launch_index] =  make_float3(  pixel_time );
#else
...
RT_PROGRAM void orthographic_camera()
{
  ...
  if( frame > 0 )
    ...
    acc_val = make_float4(prd.result, 0.f);
  output_buffer[launch_index] = acc_val * 1.0f/float(frame+1);
  accum_buffer[launch_index] = acc_val;
}
...
{
	output_buffer[launch_index] = make_float4( 1.0f, 0.0f, 0.0f, 1.0f );
}
